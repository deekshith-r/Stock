...
def developer_info_view(request):
    return render(request, 'dev.html')  # Assuming 'dev.html' is in your templates directory
...
# Sentiment analysis function
def sentiment_analysis(pcr_value):
    # Sentiment analysis logic
    if pcr_value > 1.2:
        return 'Overbought'
    elif pcr_value < 0.8:
        return 'Oversold'
    elif 0.8 <= pcr_value <= 1.2:
        return 'Balanced'
    else:
        return 'Unknown'
...
def index(request):
    ...
    data = yf.download(tickers=['AAPL', 'NFLX', 'QCOM', 'META', 'NVDA', 'MSFT'], period='1mo', interval='1d')
    data.reset_index(level=0, inplace=True)

    # Updated PCR values for each stock
    pcr_values = {'AAPL': 1.25, 'NFLX': 0.65, 'QCOM': 1.1, 'META': 1.3, 'NVDA': 0.9, 'MSFT': 1.05}

    # Sentiment analysis for each stock based on PCR values
    sentiments = {}
    for ticker, pcr_value in pcr_values.items():
        sentiment = sentiment_analysis(pcr_value)
        sentiments[ticker] = sentiment

    # Placeholder plot_div HTML code
    plot_div = "Your plot's HTML here"

    return render(request, 'index.html', {'plot_div': plot_div, 'sentiments': sentiments})

def result_view(request):
    # Placeholder PCR value and confidence score (replace with actual calculations)
    pcr_value = 1.2
    confidence = 0.726128078323486

    # Convert confidence to percentage format
    confidence_percent = "{:.2%}".format(confidence)

    # Sentiment analysis based on PCR value
    sentiment = sentiment_analysis(pcr_value)

    context = {
        'sentiment': sentiment,
        'confidence': confidence_percent,  # Use formatted confidence value
    }
...
def index(request):
    ...
    data = yf.download(
        ...
        # passes the ticker
        tickers=['AAPL', 'NFLX', 'QCOM', 'META', 'NVDA', 'MSFT'],
    ...
    fig_left.add_trace(
                go.Scatter(x=data['Date'], y=data['NFLX']['Adj Close'], name="NFLX")
            )
    fig_left.add_trace(
...
def predict(request, ticker_value, number_of_days):
    ...
    closing_price = df['Close'].iloc[-1]  # Get the last closing price from your data
    predicted_price = forecast[-1]  # Get the predicted price for the last day

    # Calculate the 5% increase threshold
    threshold = closing_price * 0.05

    # Action recommendation logic
    if predicted_price > closing_price + threshold:
        action = 'Our analysis indicates a potential increase of more than 5%, signaling a Buy recommendation.'
    elif predicted_price < closing_price:
        action = 'Based on our projections, a decrease is anticipated, indicating a Sell recommendation.'
    else:
        action = 'Considering the current market conditions, we advise maintaining positions with a Hold recommendation.'

    # Sentiment analysis based on PCR value (replace pcr_value with appropriate value)
    pcr_value = 1.2  # Replace this with the appropriate value for sentiment analysis
    sentiment = sentiment_analysis(pcr_value)
